function updatePage(data) {
    ...
    const nowSingingTimeElem = document.getElementById("now-singing-time");
    if (nowSingingElem && nowSingingTimeElem) {
        if (data.singing && data.singing.length > 1) {
            nowSingingElem.textContent = data.singing[0];
            nowSingingTimeElem.textContent = data.singing[1];
        } else {
            nowSingingElem.textContent = data.singing[0];
        }
    }

    const reserveListElem = document.getElementById("reserve-list");
    if (reserveListElem) {
        reserveListElem.innerHTML = "";
        if (data.reserve.length > 0) {
            data.reserve.forEach((item) => {
                ...
                reserveListElem.appendChild(li);
            });
        }
    }
...
window.addEventListener("load", () => {
    ...
    if (reserveListElem) {
        startScrollVertical(reserveListElem, reserveListInterval);
    }
});
