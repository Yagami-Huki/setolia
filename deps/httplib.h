...
inline unsigned char to_lower(int c)
{
	const static unsigned char table[256] = {
		...
		180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 224, 225, 226, 227, 228, 229, 230, 231,
		232, 233, 234, 235, 236, 237, 238, 239, 240, 241, 242, 243, 244, 245, 246, 215, 248, 249, 250, 251,
...
private:
	...
	bool connect_with_proxy(Socket &sock, std::chrono::time_point<std::chrono::steady_clock> start_time,
...
public:
	...
	int status() const { return handle_.response ? handle_.response->status : -1; }
...
public:
	...
	SSEClient(Client &client, const std::string &path) : client_(client), path_(path) {}
	...
	if (read_content_core(
		    ...
		    [&](const FormData &file) {
			    ...
			    if (file.filename.empty()) {
				    cur_field = req.form.fields.emplace(file.name, FormField{file.name, file.content,
											     file.headers});
				    is_text_field = true;
			    } else {
				      ...
				      FormDataHeader multipart_header, ContentReceiver multipart_receiver) const
{
	...
	if (!req.has_header("Content-Length") && !detail::is_chunked_transfer_encoding(req.headers)) {
		...
		if (s != INVALID_SOCKET) {
			...
			size_t to_peek = (payload_max_length_ > 0)
		...
		if (!etag.empty()) {
			...
			auto ret = detail::split_find(
				val.data(), val.data() + val.size(), ',', [&](const char *b, const char *e) {
					return std::equal(b, e, "*") || std::equal(b, e, etag.begin());
				});
...
inline ClientImpl::ClientImpl(const std::string &host, int port, const std::string &client_cert_path,
	...
	: host_(detail::escape_abstract_namespace_unix_domain(host)),
	  port_(port),
	  host_and_port_(detail::make_host_and_port_string(host_, port, ClientImpl::is_ssl())),
...
inline socket_t ClientImpl::create_client_socket(Error &error) const
{
	...
	return detail::create_client_socket(host_, ip, port_, address_family_, tcp_nodelay_, ipv6_v6only_,
							...
							const std::string &body, const std::string &content_type)
{
	...
	if (is_ssl() && handle.connection_->ssl) {
		handle.socket_stream_ = detail::make_unique<detail::SSLSocketStream>(
			handle.connection_->sock, handle.connection_->ssl, read_timeout_sec_, read_timeout_usec_,
			write_timeout_sec_, write_timeout_usec_);
	} else {
		handle.socket_stream_ = detail::make_unique<detail::SocketStream>(handle.connection_->sock,
										  read_timeout_sec_, read_timeout_usec_,
										  write_timeout_sec_,
										  write_timeout_usec_);
	}
#else
	handle.socket_stream_ = detail::make_unique<detail::SocketStream>(handle.connection_->sock, read_timeout_sec_,
	...
	if (!content_length_str.empty()) {
		handle.body_reader_.content_length = static_cast<size_t>(std::stoull(content_length_str));
	}
...
inline ssize_t ClientImpl::StreamHandle::read_with_decompression(char *buf, size_t len)
{
	...
	while (true) {
		...
		bool decompress_ok = decompressor_->decompress(compressed_buf, static_cast<size_t>(n),
...
inline bool ClientImpl::process_request(Stream &strm, Request &req, Response &res, bool close_connection, Error &error)
{
	...
	if ((res.status != StatusCode::NoContent_204) && req.method != "HEAD" && req.method != "CONNECT") {
		auto redirect = 300 < res.status && res.status < 400 && res.status != StatusCode::NotModified_304 &&
				follow_location_;
...
ClientImpl::get_multipart_content_provider(const std::string &boundary, const UploadFormDataItems &items,
					   const FormDataProviderItems &provider_items) const
{
	size_t cur_item = 0;
	size_t cur_start = 0;
...
inline Result ClientImpl::Post(const std::string &path, const Headers &headers, const std::string &body,
			       const std::string &content_type, UploadProgress progress)
{
	return send_with_content_provider_and_receiver("POST", path, headers, body.data(), body.size(), nullptr,
...
inline Result ClientImpl::Patch(const std::string &path, const Headers &headers, const std::string &body,
				const std::string &content_type, UploadProgress progress)
{
	return send_with_content_provider_and_receiver("PATCH", path, headers, body.data(), body.size(), nullptr,
						       nullptr, content_type, nullptr, progress);
}
...
inline Result Client::Post(const std::string &path, const Headers &headers,
			   ...
			   ContentReceiver content_receiver, DownloadProgress progress)
{
	return cli_->Post(path, headers, std::move(content_provider), content_type, std::move(content_receiver),
			  progress);
}
...
inline Result Client::Put(const std::string &path, const Headers &headers,
			  ...
			  ContentReceiver content_receiver, UploadProgress progress)
{
	return cli_->Put(path, headers, std::move(content_provider), content_type, std::move(content_receiver),
			 progress);
}
...
inline Result Client::Patch(const std::string &path, const Headers &headers,
			    ...
			    ContentReceiver content_receiver, UploadProgress progress)
{
	return cli_->Patch(path, headers, std::move(content_provider), content_type, std::move(content_receiver),
			   progress);
}
